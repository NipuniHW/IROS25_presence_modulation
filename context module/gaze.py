...
from context_identification import get_final_label_for_gaze, classify_real_time_audio, process_speech_to_text_and_sentiment, classify_context
import threading
import queue
...
class Gaze_Score:
    def __init__(self):
        # Initial parameters for gaze
        self.gaze_time = 0.0  # Time the person has been looking at the robot in seconds
        self.gaze_quality = 0.0  # Gaze quality, which is a percentage (0-100)
        self.max_gaze_time = 3.0  # Max time for a full gaze, e.g., 60 seconds
        self.alpha = 0.7  # Weight for gaze quality in the gaze score calculation
                    
    def update_gaze(self, gaze_time, gaze_entropy):
        """
        Updates the gaze time and quality from external input.
        """
        self.gaze_time = gaze_time
        self.gaze_quality = (1-gaze_entropy) * 100  # Converting entropy to percentage
        #print(f"Debug: update gaze_time is {self.gaze_time}")

    
    def normalize_gaze_time(self):
        """
        Normalizes the gaze time to a scale of 0-100.
        """
        return min(self.gaze_time / self.max_gaze_time, 1.0)
    
    def calculate_gaze_score(self, output_queue):
        """
        Calculates the gaze score based on time and quality.
        The gaze score will be between 0-100.
        """
        # Normalize gaze time to 0-100 range
        normalized_gaze_time = self.normalize_gaze_time()
        normalized_gaze_quality = self.gaze_quality / 100
        
        # Core logic for calculating gaze score
        if normalized_gaze_time < 0.4 and normalized_gaze_quality > 0.7:
            # Low time, high quality -> low score
            gaze_score = normalized_gaze_quality * 30
        elif 0.4 <= normalized_gaze_time <= 0.7 and normalized_gaze_quality <= 0.5:
            # Moderate time, low quality -> medium score
            gaze_score = 40 + normalized_gaze_time * 30
        elif normalized_gaze_time > 0.7 and normalized_gaze_quality <= 0.3:
            # High time, very low quality -> high score
            gaze_score = 70 + normalized_gaze_time * 20
        else:
            # Weighted average of time and quality for general cases
            gaze_score = (self.alpha * normalized_gaze_quality + (1 - self.alpha) * normalized_gaze_time) * 100

        # Ensure score is within the range [0, 100]
        gaze_score = min(max(gaze_score, 0), 100)
        output_queue.put(gaze_score)

def sync_context(output_queue, output_queue2):
    sr = 16000 
    gaze_system = Gaze_Score()
    while True:
...
def main():
    ...
    gaze_score_calculator = Gaze_Score()
    
    # Start threads
    gaze_thread_obj = threading.Thread(target=gaze_score_calculator.calculate_gaze_score, args=(output_queue, attention_window, gaze_score_calculator), daemon=True)
    audio_context_thread_obj = threading.Thread(target=sync_context, args=(output_queue,output_queue2), daemon=True)
    ...
    while cap.isOpened():
        ...
        gaze_score_calculator.update_gaze(metrics["gaze_time"], metrics["gaze_entropy"])
        gaze_score_calculator.normalize_gaze_time()
        
        # Calculate gaze score (only if 3 seconds have passed)
        if current_time - last_time >= 3:
            gaze_score_calculator.calculate_gaze_score(output_queue)
        ...
        #    print(f"Gaze Score: {gaze_score}, Final Context: {final_label}")
